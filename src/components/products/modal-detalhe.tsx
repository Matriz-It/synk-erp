...
import { AlertTriangle, ArrowDownRight, ArrowUpRight, Layers, Loader2, Plus, Settings } from 'lucide-react'
    ...
    <ModalWrapper open={open} onClose={onClose} title={produto.nome} width="max-w-2xl">
      <div className="space-y-5 p-4 sm:p-6">
        ...
        {produto.isMateriaPrima && (
          <div className="flex items-center gap-2 rounded-lg border border-[#fef3c7] bg-[#fffbeb] px-3 py-2">
            ...
            <p className="text-[13px] font-semibold text-[#92400e]">
              Matéria-Prima — usada na composição de outros produtos
            </p>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {!produto.isMateriaPrima && onComposicao && (
            <button
              type="button"
              onClick={onComposicao}
              className="flex items-center gap-1.5 rounded-md border border-[#dbeafe] bg-[#eff6ff] px-3 py-2 text-[13px] font-semibold text-[#2563eb] transition-colors hover:bg-[#dbeafe]"
            >
              <Layers className="size-4" strokeWidth={1.5} />
              Composição
              {componentesCount > 0 && (
                <span className="ml-1 rounded-full bg-[#2563eb] px-1.5 py-0.5 text-[10px] font-bold text-white">
                  {componentesCount}
                </span>
              )}
            </button>
        ...
        {/* Info de composição */}
        {!produto.isMateriaPrima && componentesCount > 0 && (
          <div className="flex items-center gap-2 rounded-lg border border-[#dbeafe] bg-[#eff6ff] px-3 py-2">
            <Layers className="size-4 text-[#2563eb]" strokeWidth={1.5} />
            <p className="text-[12px] text-[#1e40af]">
              Este produto usa <strong>{componentesCount} matéria{componentesCount !== 1 ? 's-primas' : '-prima'}</strong> na produção.
              Ao dar entrada no estoque, os estoques das matérias-primas são descontados automaticamente.
            </p>
          </div>
        )}
